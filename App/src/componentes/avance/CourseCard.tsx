...
const estadoStyles: Record<NonNullable<Props["estado"]>, string> = {
    ...
    REPROBADO: "bg-red-50 border-red-200",
    INSCRITO: "bg-yellow-50 border-yellow-200",
    PENDIENTE: "bg-white border-slate-200",
};

export const CourseCard: FC<Props> = ({ nombre, codigo, creditos, estado="PENDIENTE", nf, intentos }) => (
    <div className={`rounded-lg border shadow-sm px-3 py-2 ${estadoStyles[estado]}`}>
        ...
        <div className="flex items-center justify-between text-[11px]">
            {/* NF */}
            {typeof nf === "number" ? (
                <span className="inline-flex items-center gap-1 bg-white/70 border border-black/10 rounded-full px-1.5 py-0.5 leading-none">
