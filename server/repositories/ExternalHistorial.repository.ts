const HISTORIAL_URL = "https://puclaro.ucn.cl/eross/avance/avance.php";

type FetchResponse = {
  json: () => Promise<any>;
  ok: boolean;
  status: number;
};

export class ExternalHistorialAPI {
  static async fetchHistorial(rut: string, codcarrera: string) {
    const url = new URL(HISTORIAL_URL);
    url.search = new URLSearchParams({ rut, codcarrera }).toString();

    const response = (await fetch(url.toString())) as FetchResponse;
    const data = await this.parseJson(response);

    if (!response.ok) {
      const error = new Error(
        `External Historial API HTTP ${response.status}${data?.error ? ` - ${data.error}` : ""}`,
      );
      (error as any).statusCode = 502;
      (error as any).isExternal = true;
      throw error;
    }

    return data;
  }

  private static async parseJson(response: FetchResponse) {
    try {
      return await response.json();
    } catch (error) {
      const parsingError = new Error("External Historial API response inválida");
      (parsingError as any).statusCode = 502;
      (parsingError as any).isExternal = true;
      throw parsingError;
    }
  }
}
